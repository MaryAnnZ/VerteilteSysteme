	...
	public void run() {
		while (this.client.running) {
			try {
				...
				case "login":
					responseMap.put("login", response.substring(parts[0].length() + 1));
					break;

				case "logout":
					responseMap.put("logout", response.substring(parts[0].length() + 1));
					break;

				case "send":
					responseMap.put("send", response.substring(parts[0].length() + 1));
					break;

				case "public":
					client.writePublicMessage(response.substring(parts[0].length() + 1));
					break;

				case "register":
					responseMap.put("register", response.substring(parts[0].length() + 1));
					break;

				case "lookup":
					responseMap.put("lookup", response.substring(parts[0].length() + 1));
					break;
